...
Alerts use one line per changed option: `🟢 iPhone 18 Pro Max 256GB Black · World Trade Center · Available Today (Sep 20)` or `🔴 iPhone 18 Pro Max 256GB Black · World Trade Center · Unavailable`. The default policy allows **any available pickup date**, including tomorrow; switch to **Today only** in Settings if desired.

Alerts are sent on the first available observation allowed by your pickup policy, when a previously available model becomes unavailable at that store, and when it restocks. Items initially observed as unavailable stay quiet, as do repeated checks of unchanged stock. Changes to the pickup date or quote do not repeat an acknowledged availability alert. Notification history survives restarts and disabling/re-enabling a selection; observed unavailability resets that store/product's availability acknowledgement. Failed requests preserve the previous result as stale instead of marking the product unavailable. Failed Slack delivery remains pending for retry after a fresh observation, including out-of-stock alerts and transitions observed by checks without `--notify`. If an item restocks before an out-of-stock alert is delivered, only its current availability is reported, subject to your pickup policy. Delivery is at-least-once: a crash or network timeout after Slack accepts a message but before local acknowledgement can produce a duplicate.
