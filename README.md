...
- Receive Slack alerts when a model becomes available at a store, without repeats until it goes out of stock and returns.
...
Alerts track each product/store pair. After a successful alert, pickup date or quote changes do not trigger another; the monitor must observe that pair out of stock before alerting on a restock. Failed API requests remain unknown instead of becoming “out of stock.” Failed Slack deliveries are retried. Requests are paced, overlapping checks are locked out, and errors trigger backoff.
